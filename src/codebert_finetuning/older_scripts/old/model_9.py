...
log_file = os.path.join(log_dir, "model_9_log.txt")
...
logging.info(" Starting final training script with fixed dataset and better generalisation...")
...
class FixedRobertaForSequenceClassification(RobertaForSequenceClassification):
    def __init__(self, config, class_weights=None):
        ...
        self.dropout = nn.Dropout(p=0.4)  # Adjusted dropout 
...
# ---------------------------
class_weights = torch.tensor([1.0, 1.0]).to(device)  # Balanced dataset, so equal weighting
