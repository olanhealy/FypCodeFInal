...
log_file = os.path.join(log_dir, "fixed_training_log.txt")
...
logging.info(" Starting updated training script with fixed overfitting issues...")
...
class FixedRobertaForSequenceClassification(RobertaForSequenceClassification):
    def __init__(self, config, class_weights=None):
        ...
        self.dropout = nn.Dropout(p=0.5)  # Higher dropout for regularisation
...
def augment_input(text):
    """Randomly shuffle or remove commit messages to prevent memorisation."""
    if "Commit Summary:" in text:
        if random.random() < 0.5:
            text = text.replace("Commit Summary:", "Description:")
        if random.random() < 0.3:
            text = text.replace("Commit Summary: ", "")  # Remove message
    return text
